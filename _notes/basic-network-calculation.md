...
Take the octet where there is the split between `0` and `1` -> `11111000`. Count the number of bits (`1`), in the example there are 5 bits that are used to identify the subnet. To find the total number of subnets available simply raise 2 to the power of `5` like that: <img src="https://tex.s2cms.ru/svg/2%5E5" alt="2^5" /> = 32. The number of subnet is 32.
...
Raise `2` to the power of `32` minus the mask of the network, then subtract `2` like that: <img src="https://tex.s2cms.ru/svg/2%5E%7B32-29%7D-2" alt="2^{32-29}-2" /> = 6. The number of hosts is 6.
...
`/29` -> `11111111.11111111.11111111.11111000`
Take the octet where there is the split between `0` and `1` -> `11111000` and complement to 1 -> `00000111`.
...
`1010|1111.11111111` replace the right part by one<br/>
